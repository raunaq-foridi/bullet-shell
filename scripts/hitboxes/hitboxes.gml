...
//Windboxes - move the enemy without causing damage or iframes. Basically pogoboxes on enemies

function p_windbox(_rel_x,_rel_y,_width,_height,_lifespan,_wind_x,_wind_y,_shape="box",_delay=0,_follow_player=false,_vel_x=0,_vel_y=0){
	
	var _windbox = instance_create_layer(o_player.x+_rel_x,o_player.y+_rel_y,"Instances",o_player_windbox);
	...
	with(_windbox){
		...
		switch(shape){
			case "box":
				sprite_index = s_windbox_square;
			break
			case "circle":
				sprite_index = s_windbox_circle;
			break
			default:
				sprite_index = s_windbox_square;
			break
